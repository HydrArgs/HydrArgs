...
namespace HydrArgs::Backend{
	...
	struct CLIPPBackend: public IArgsParser{
		...
		CLIPPBackend(const std::string &name, const std::string &descr, const std::string &usage [[maybe_unused]], std::vector<Arg*> dashedSpec, std::vector<Arg*> positionalSpec, Streams streams): IArgsParser(streams), name(name), descr(descr){
		...
		virtual ~CLIPPBackend() override = default;
		...
		virtual ParseResult _parseArgs(CLIRawArgs rawArgs) override {
			...
			decltype(clipp::parse(0, static_cast<char **>(nullptr), app)) parseRes;
			{
				...
				parseRes = clipp::parse(tempArgvVec.size() - 1, const_cast<char **>(tempArgvVec.data()), app);
			}
