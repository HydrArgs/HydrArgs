...
namespace HydrArgs::Backend{
	...
	struct ParseEvents parseCommonOptions(CLIRawArgs::VectorT & argvVector){
		auto parsed = boost::program_options::command_line_parser(argvVector.size() - 1, argvVector.data()).options(oCommonOptsDesc).allow_unregistered().run();
	...
	struct BoostArgsBackend: public IBackendOwnStoredSpec{
		...
		virtual ParseResult _parseArgs(CLIRawArgs rawArgs) override {
			...
			try{
				auto parsed = boost::program_options::command_line_parser(tempArgvVec.size() - 1, tempArgvVec.data()).options(oDesc).positional(pd).allow_unregistered().run();
