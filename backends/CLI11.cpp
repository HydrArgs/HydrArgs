...
namespace HydrArgs::Backend{
	...
	struct HYDRARGS_BACKEND_API CLI11Backend: public IArgsParser{
		...
		CLI11Backend(const std::string &name, const std::string &descr, const std::string &usage [[maybe_unused]], std::vector<Arg*> dashedSpec, std::vector<Arg*> positionalSpec, Streams streams): IArgsParser(streams), app(name, descr){
		...
		virtual ~CLI11Backend() override = default;
		...
		void _printHelp(std::ostream &stream, const char * const argv0 [[maybe_unused]], CLI::AppFormatMode mode) {
