...
namespace HydrArgs::Backend{
	struct QCommandLineBackend: public IBackendOwnStoredSpec, public PositionalParserIterative{
		...
		virtual ~QCommandLineBackend() override = default;
		virtual ParseResult _parseArgs(CLIRawArgs rawArgs) override {
			...
			// make sure QCoreApplication is created. If it is not, create it and recurse
			{
				...
				if(appPtr == nullptr){
					...
					int shitArgc = 1;
					QCoreApplication app(shitArgc, const_cast<char **>(&rawArgs.argv0));
