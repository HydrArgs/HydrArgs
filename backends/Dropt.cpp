...
namespace HydrArgs::Backend{
	...
	template <dropt_option_handler_decl f> dropt_error dropt_handle_mandatory(dropt_context *context, const dropt_option *option, const dropt_char *optionArgument, void *dest){
		dropt_error_ec res = static_cast<dropt_error_ec>(f(context, option, optionArgument, dest));
	...
	struct DroptBackend: public IArgsParser, public PositionalParserSpannable{
		...
		DroptBackend(const std::string &name, const std::string &descr, const std::string &usage [[maybe_unused]], std::vector<Arg*> dashedSpec, std::vector<Arg*> positionalSpec, Streams streams): IArgsParser(streams), PositionalParserSpannable(this), usage(usage), descr(descr){
		...
		virtual bool isSealed() const override {
			return nativeArgs.size() && nativeArgs.back().handler == nullptr;
		}
		...
		void _addDashedArg(Arg *argPtr){
			...
			opt.attr = static_cast<DroptAttrIntT>(dropt_attr::our_mandatory) * bool(argPtr->minCount);
		...
		virtual ~DroptBackend() override = default;
		...
		virtual ParseResult _parseArgs(CLIRawArgs rawArgs) override {
			...
			char **rest = droptContext.parse(argsVector.size() - 1, const_cast<dropt_char **>(&argsVector[1]));
			restc = 0;
			...
			dropt_error_ec droptNativeErr = static_cast<dropt_error_ec>(droptContext.get_error());
