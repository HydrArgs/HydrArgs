...
namespace HydrArgs::Backend{
	struct LyraBackend: public IArgsParser{
		...
		LyraBackend(const std::string &name, const std::string &descr, const std::string &usage [[maybe_unused]], std::vector<Arg*> dashedSpec, std::vector<Arg*> positionalSpec, Streams streams): IArgsParser(streams), app(){
		...
		virtual ParseResult _parseArgs(CLIRawArgs rawArgs) override {
			...
			lyra::parse_result result = resultOpt.value();

			PROCESS_CASE_OF_HELP_CALLED(show_help);

			if(!result){
				PROCESS_CASE_OF_SYNTAX_ERROR_MESSAGE(result.errorMessage());
			}
