...
struct DLibBackend: public IBackendOwnStoredSpec, public PositionalParserIterative{
	std::string name;
	std::string usage;
	...
	DLibBackend(const std::string &name, const std::string &descr, const std::string &usage, std::vector<Arg*> dashedSpec, std::vector<Arg*> positionalSpec, Streams streams): IBackendOwnStoredSpec(dashedSpec, positionalSpec, streams), PositionalParserIterative(this), name(name), usage(usage){
	...
	virtual ParseResult _parseArgs(CLIRawArgs rawArgs) override {
...

		{
			...
			app.parse(tempArgvVec.size(), tempArgvVec.data());
		}
