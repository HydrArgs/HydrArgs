...
namespace HydrArgs::Backend{
	...
	struct OctargsBackend: public IBackendOwnStoredSpec{
		...
		OctargsBackend(const std::string &name [[maybe_unused]], const std::string &descr, const std::string &usage [[maybe_unused]], std::vector<Arg*> dashedSpec, std::vector<Arg*> positionalSpec, Streams streams): IBackendOwnStoredSpec(dashedSpec, positionalSpec, streams){
		...
		virtual ~OctargsBackend() override = default;
		...
		virtual ParseResult _parseArgs(CLIRawArgs rawArgs) override {
			...
			try{
				...
				res = app.parse(tempArgvVec.size() - 1, tempArgvVec.data());
	...
	IArgsParser * argsParserFactory(const std::string &name, const std::string &descr, const std::string &usage [[maybe_unused]], std::vector<Arg*> dashedSpec, std::vector<Arg*> positionalSpec, Streams streams){
		return new OctargsBackend(name, descr, usage, dashedSpec, positionalSpec, streams);
	}
};
