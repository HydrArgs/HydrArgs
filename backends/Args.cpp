...
struct ArgsBackend: public IBackendOwnStoredSpec{
		...
		if(isPositional){
			...
			switch(argPtr->type){
				case ArgType::flag:
				{
					...
					nativeArgs.emplace_back(std::make_unique<args::ValueFlag<decltype(specOptPtr->value)>>(app, argPtr->name, argPtr->description, {argPtr->name, argPtr->letter}, specOptPtr->value, args::Options::Required | args::Options::Single));
				}
				break;
				case ArgType::s4:
				{
	...
	virtual ParseResult _parseArgs(CLIRawArgs rawArgs) override {
		try {
			...
			app.ParseCLI(tempArgvVec.size() - 1, tempArgvVec.data());
