	...
	struct GetoptBackend: public IBackendOwnStoredSpec, PositionalParserIterative{
		...
		virtual ParseResult _parseArgs(CLIRawArgs rawArgs) override {
			...
			for(;;){
				...
				short_option_char = getopt_long(tempArgvVec.size() - 1, const_cast<char **>(tempArgvVec.data()), nativeShortArgs.data(), nativeLongArgs.data(), &long_option_index);
					...
					default:
					{
						...
						if(long_option_index){
							...
							if(long_option_index >= LONG_ARGS_OFFSET){
								auto dashedArgIdx = long_option_index - LONG_ARGS_OFFSET;
						...
						if(!argPtr){
							...
							auto it = dashedCharToArgMap.find(short_option_char);
